...
const asyncTimeFunction = async function* (timeParams) {
  let timerCount = timeParams;
  while (true) {
...
const asyncFunctionInitiator = async () => {
  const asyncTimeObject = asyncTimeFunction(100);
  for await (const timeValue of asyncTimeObject) {
    console.log(timeValue);
  }
};

asyncFunctionInitiator();
